...
GPIO_INTERRUPT = 17
#인터럽트 함수가 호출되면 GPIO 값을 읽는다.
def myInterrupt(channel): 
	val = bus.read_byte_data(dID, MCP23008_INTF)
	print "Channel:", channel, "Interrrupt Occured ->By   GPIO:", byte_dec2bin(val)
	if(0 == val):
		return
	val = bus.read_byte_data(dID, MCP23008_INTCAP)
	print "Channel:", channel, "Interrrupt Occured ->Read GPIO:", byte_dec2bin(val)
...
bus.write_byte_data(dID, MCP23017_IODIRB, 0x00)	#0x01 address
# 입력모드 핀에 인터럽트를 설정한다.
bus.write_byte_data(dID, MCP23008_GPINTEN, 0xF0)
bus.write_byte_data(dID, MCP23008_INTCON, 0x00)
bus.write_byte_data(dID, MCP23008_GPPUA, 0xF0)
# IOCON(binary 00101000)
bus.write_byte_data(dID, MCP23008_IOCON, 0x28)


print_mcp23008()
# ON->OFF, OFF->ON 모두 이벤트 발생
GPIO.add_event_detect(GPIO_INTERRUPT, GPIO.BOTH, callback=myInterrupt) 
