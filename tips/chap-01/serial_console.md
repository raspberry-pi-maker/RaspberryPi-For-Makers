...
## Windows 11 사용자용 FTDI 칩셋 제품 
Windows 11 사용자들은 FTDI사 칩셋을 사용한 제품을 사용하는 것이 가장 무난하다. Prolific사 칩셋 제품보다는 대체로 조금 비싸지만 알리익스프레스에서 잘 찾아보면 2000원 이내의 제품도 있다.

다음은 알리 익스프레스에서 약 1500원에 구매한 FTDI칩셋을 사용한 제품이다. 사용전 한가지 주의점은 제품 핀헤더 윗쪽의 점퍼 세팅이다. 대부분 최초 5V로 세팅한 상태로 제공된다. 앞에서 설명한 것처럼 라즈베리파이에서 사용하려면 3.3V 로직 레벨을 사용해야 하기 때문에 점퍼 위치를 확인 후 변경한다. 점퍼를 뽑으면 아주 작은 글씨가 보인다. 로직레벨에 대해서는 뒤에서 다시 설명한다 <br/> 

![FTDI 제품](../../tip_image/1-serial-15.png)

<br/> <br/>
이 제품을 USB 케이블에 연결한 다음 Windows 11이 설치된 PC에 연결한다. 처음 드라이버 설치하는 동안에 장치관리자에서 "Other Devices" 항목에 경고 표시와 함께 나타날 수 있다. 하지만 잠시 후 드라이버가 설치되면 다음과 같이 정상으로 인식되는 것을 확인할 수 있다. <br/> 

![FTDI 제품](../../tip_image/1-serial-16.png)

<br/> 

Windows 11에서 시리얼 포트가 인식되었기 때문에 이제 다음 작업은 Windows 10, 11 모두 동일하다. 
<br/> <br/>
...
위 그림에서 3.3V용 TTL 신호 역시 5V에서도 작동하는 것을 알 수 있다. 대부분의 USB-TTL 제품이 이러한 점을 고려해 3.3V 로직 레벨을 사용하고 있지만 사용전에 반드시 제품의 데이터시트를 확인하거나 설명서에 3.3V를 사용하는 라즈베리파이에서 사용가능한지 여부를 확인하는 것이 안전하다. 5V 로직레벨의 경우 2.7V 이상만 규정하고 있기 때문에 라즈베리파이에서 감당할 수 있는 3.3V 이상의 시그널 전압이 들어올 수 있기 때문이다.

그리고 이 이유로 5V 신호를 사용하는 아두이노와 라즈베리파이를 UART를 이용해 직접 통신하려면 반드시 Rx, Tx 선은 가운데 로직 레벨 쉬프트를 경유해야 한다. 
...
다음 그림처럼 3개의 듀폰 케이블 GND, Rx, Tx 핀을 연결한다. 라즈베리파이는 자체 3.3V 전원을 사용해 Rx, Tx를 처리하기 때문에 3.3V를 연결할 필요는 없으며 GND 핀을 연결해 서로 GND 전압을 맞춰주면 된다. <br/> 
