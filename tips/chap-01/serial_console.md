...
다음은 알리 익스프레스에서 약 1500원에 구매한 FTDI칩셋을 사용한 제품이다. 사용전 한가지 주의점은 제품 핀헤더 윗쪽의 점퍼 세팅이다. 대부분 최초 5V로 세팅한 상태로 제공된다. 라즈베리파이에서 사용하려면 3.3V 로직 레벨을 사용해야 하기 때문에 점퍼 위치를 확인 후 변경한다. 점퍼를 뽑으면 아주 작은 글씨가 보인다. 로직레벨에 대해서는 뒤에서 다시 설명한다 <br/> 
